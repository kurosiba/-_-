...
func main() {
	...
	timeOutServerCount := 0           //タイムアウトしているサーバーの数
	...
	*一行ずつログファイルを読み取り、カンマごとに分割し、アドレスと復帰時間をコンソールに出力する。
	...
	for scanner.Scan() { //ログファイルを一行ずつ読む
		 ...
		 *タイムアウトや故障に関する部分
		 */
		for i := 0; i < timeOutServerCount; i++ { //同じアドレスから連続でタイムアウトが出ていないかの確認
			serverChecker = false
		...
		if arry1[2] == "-" && serverChecker == false { //タイムアウトが出たサーバーのタイムスタンプとアドレス、タイムアウトの回数をlogData[]に代入している(初期化)
			...
			timeOutLogData[timeOutServerCount-1][0] = arry1[0]
			timeOutLogData[timeOutServerCount-1][1] = arry1[1]
			timeOutLogData[timeOutServerCount-1][2] = str
		}

		for i := 0; i < timeOutServerCount; i++ {
			timeOutCount, _ := strconv.Atoi(timeOutLogData[i][2])
			...
			if arry1[1] == timeOutLogData[i][1] && timeOutCount >= N { //サーバーが復帰したタイミングでアドレスと故障時間をコンソールに出力する
				...
				if arry1[2] != "-" {
					...
					var timeOutSubnet []string
					var subnet []string

					for i := 0; i < timeOutServerCount; i++ { //タイムアウト回数がN以上のサーバーのサブネットの値を取り出している
						timeOutSubnet = strings.Split(timeOutLogData[i][1], "/")
					}
					for i := range logData { //タイムアウトしていないサーバーのサブネットの値を取り出している
						subnet = strings.Split(logData[i][0], "/")
					}

					if timeOutSubnet[0] != subnet[0] {
						fmt.Println(timeOutSubnet[1] + " is failure")
					}
